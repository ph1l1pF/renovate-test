...
LABEL_CHECK_ISSUE_EXISTS=$(gh search issues --author "bot" --repo $REPO --json "number,title" | jq --arg title "$ISSUE_TITLE" 'map(select(.title == $title))') || { echo "Failed to fetch existing label check issue"; exit 1; }
