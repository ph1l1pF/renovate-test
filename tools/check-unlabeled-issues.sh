...
LABEL_CHECK_ISSUE_EXISTS=$(gh search issues --repo $REPO --json "number,title" | jq --arg title "$ISSUE_TITLE" 'map(select(.title == $title and .author.is_bot == "true"))') || { echo "Failed to fetch existing label check issue"; exit 1; }
